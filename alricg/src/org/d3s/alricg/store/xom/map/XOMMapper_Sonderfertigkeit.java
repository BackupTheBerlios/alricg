...
class XOMMapper_Sonderfertigkeit extends XOMMapper_Fertigkeit implements XOMMapper {
    ...
    public void map(Element xmlElement, CharElement charElement) {
        ...
        try {
            ...
            Element current = xmlElement.getFirstChildElement("permAsP");
            if (current != null) {
                sonderfertigkeit.setPermAsp(Integer.parseInt(current.getValue()));
            }

            // Auslesen der permanenten Karmaenergie Kosten
            current = xmlElement.getFirstChildElement("permKa");
            if (current != null) {
                sonderfertigkeit.setPermKa(Integer.parseInt(current.getValue()));
            }

            // Auslesen der permanenten Lebensenergie Kosten
            current = xmlElement.getFirstChildElement("permLeP");
            if (current != null) {
                sonderfertigkeit.setPermLep(Integer.parseInt(current.getValue()));
            }
    ...
    public void map(CharElement charElement, Element xmlElement) {
        ...
        if (apKosten != CharElement.KEIN_WERT
                && apKosten != FormelSammlung.berechneSfAp(sonderfertigkeit.getGpKosten())) {
