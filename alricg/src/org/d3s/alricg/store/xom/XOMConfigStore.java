...
public class XOMConfigStore implements ConfigStore {
     ...
     * 			NullPointerException - Wenn nicht 30 Elemente Vorhanden sind
     * @author V.Strelow
     */
    private HashMap<KostenKlasse, Integer[]> loadSkt(Element element) 
    	...
    	for (int i = 0; i < 32; i++) {
    		...
    		sktMap.get(KostenKlasse.A)[i] = 
    			Integer.parseInt(tmpElement.getAttributeValue("A"));
    		sktMap.get(KostenKlasse.B)[i] = 
    			Integer.parseInt(tmpElement.getAttributeValue("B"));
    		sktMap.get(KostenKlasse.C)[i] = 
    			Integer.parseInt(tmpElement.getAttributeValue("C"));
    		sktMap.get(KostenKlasse.D)[i] = 
    			Integer.parseInt(tmpElement.getAttributeValue("D"));
    		sktMap.get(KostenKlasse.E)[i] = 
    			Integer.parseInt(tmpElement.getAttributeValue("E"));
    		sktMap.get(KostenKlasse.F)[i] = 
    			Integer.parseInt(tmpElement.getAttributeValue("F"));
    		sktMap.get(KostenKlasse.G)[i] = 
    			Integer.parseInt(tmpElement.getAttributeValue("G"));
    		sktMap.get(KostenKlasse.H)[i] = 
    			Integer.parseInt(tmpElement.getAttributeValue("H"));	
    	}
    ...
    public HashMap<KostenKlasse, Integer[]> getSkt() throws ConfigurationException {
        ...
        final Element configRoot = XOMHelper
        					.getRootElement(new File(props.getProperty("config.file")))
		    ...
		    for (int i = 0; i < elements.size(); i++) {
		    	...
		    	if (elements.get(i).getAttributeValue("id").equals(pack)) {
		    		...
		    		if ( elements.get(i).getFirstChildElement("skt") == null
		     				|| elements.get(i).getFirstChildElement("skt")
		    							.getAttributeValue("useOriginal")
		    							.equals("true") ) {
