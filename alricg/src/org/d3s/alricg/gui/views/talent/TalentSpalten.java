...
public class TalentSpalten implements SpaltenSchema {
    public enum Spalten {
        name("Name"), stern("*"), stufe("-"), modis("-"), sorte("Sorte"), kostenKlasse("SKT"), kosten("-"), art("Art"), spezialisierungen(
                "-"), auswahl("-"), leitTalent("-"), probe("Probe"), plus("+"), minus("-");
    ...
    public Enum[] getSpalten(SpaltenArt art) {
        ...
        switch (art) {
        case objektDirekt:
            return new Spalten[] { Spalten.name, // 0
                    Spalten.stern, // 1
                    Spalten.sorte, // 2
                    Spalten.art, // 3
                    Spalten.kostenKlasse, Spalten.probe, Spalten.plus, };
    ...
    public Comparator<?> getComparator(Object column) {
        ...
        case name:
            return namensComparator;
        case sorte:
            return TalentSpalten.compSorte;
        case art:
            return TalentSpalten.compArt;
        case kostenKlasse:
            return TalentSpalten.compKostenKlasse;
        }
    ...
    private static Comparator compSorte = new Comparator<Talent>() {
        public int compare(Talent arg0, Talent arg1) {
            return arg0.getSorte().toString().compareTo(arg1.getSorte().toString());
        }
    };

    private static Comparator compArt = new Comparator<Talent>() {
        public int compare(Talent arg0, Talent arg1) {
            return arg0.getArt().toString().compareTo(arg1.getArt().toString());
        }
    };

    private static Comparator compKostenKlasse = new Comparator<Talent>() {
        public int compare(Talent arg0, Talent arg1) {
