 ...
 * @author V. Strelow
 */
public class SortableTreeTableModel<E> extends AbstractTreeTableModel implements ProzessorObserver {
	private final Object[] columns; // Die Spalten-Titel
	private final SpaltenSchema spaSchema; // Spezifische Methoden für die Spalten
	private final ZeilenSchema zeilenSchema; // Spezifische Methoden für Typ <E>
	...
	public SortableTreeTableModel(SpaltenSchema sSchema, ZeilenSchema wSchema, SpaltenArt art, String rootText) {
		super(new DefaultMutableTreeNode(rootText));
    	
		this.columns = sSchema.getSpalten(art);
		this.spaSchema = sSchema;
		this.zeilenSchema = wSchema;
	 ...
	 * @return Das benutze Schema dieses Modells
	 */
	public ZeilenSchema getWorkSchema() {
		return zeilenSchema;
	}
	 ...
	 */
	protected void ordneNachOrdnern(List<E> elemListe,
										DefaultMutableTreeNode nodeToAdd) 
	{
		Object[] ordner = zeilenSchema.getSortOrdner();
...
//		passend zur Ordner
		for (int i = 0; i < elemListe.size(); i++) {
			tmpIntArray = zeilenSchema.getOrdinalFromElement(elemListe.get(i));
...
//			 Fall dieser ordner keine Elemente enthalten würde, wird er nicht angelegt
	...
	private void ordneNachSammelbegriff(ArrayList<DefaultMutableTreeNode> array) {
		...
		if (!zeilenSchema.hasSammelbegriff()) {
			return;
		}
	...
	*/
	...
	public void sortTableByColumn(int column) {
		...
		nodeComp.setRealComparator(zeilenSchema.getComparator(columns[column]));
    ...
    public boolean isCellEditable(Object node, int column) {
    	...
    	return zeilenSchema.isCellEditable(
	...
	public void setValueAt(Object aValue, Object node, int column) {
		...
		zeilenSchema.setCellValue(aValue, 
				((DefaultMutableTreeNode) node).getUserObject(), 
				columns[column]);
	}
	...
	public Object getValueAt(Object node, int column) {
		...
		return zeilenSchema.getCellValue(mutNode.getUserObject(), columns[column]);
	}
