...
public class EigenschaftBoxGenTest extends TestCase {
    ...
    private GeneratorLink getLink(EigenschaftEnum enu) {
        ...
        return (GeneratorLink) prozessor.getLinkById(enu.getId(), null, null, CharKomponente.eigenschaft);
    ...
    public void testUpdateStufe() {
        ...
        assertEquals(332, // GesamtKosten
                ((EigenschaftBoxGen) prozessor.getHeld().getElementBox(CharKomponente.eigenschaft))
                        .getGesamtTalentGpKosten());

        // Sonstiges:
        assertEquals( // Sollte nix kosten!
                0, getLink(EigenschaftEnum.MR).getKosten());

        assertEquals( // Max Wert = IN
                12, prozessor.getMaxWert(getLink(EigenschaftEnum.KA)));
    }
    ...
    public void testMU() {
        ...
        link.setWert(2);
        link.setZielId(data.getCharElement(EigenschaftEnum.MU.getId()));
    ...
    public void testKL() {
        ...
        link = new IdLink(k, null);
        link.setWert(2);
        link.setZielId(data.getCharElement(EigenschaftEnum.KL.getId()));
        // Profession setzen
        link2 = new IdLink(p, null);
        link2.setWert(2);
        link2.setZielId(data.getCharElement(EigenschaftEnum.KL.getId()));
    ...
    public void testLEP() {
        ...
        link = new IdLink(k, null);
        link.setWert(4);
        link.setZielId(data.getCharElement(EigenschaftEnum.LEP.getId()));
        ...
        assertEquals( // (10 + 10 + 11) / 2 = 16 + 4 + 3 = 23
                23, prozessor.getHeld().getEigenschaftsWert(EigenschaftEnum.LEP));
    ...
    public void testASP() {
        ...
        link = new IdLink(k, null);
        link.setWert(1);
        link.setZielId(data.getCharElement(EigenschaftEnum.ASP.getId()));
        // Setzen eines Modis durch Profession
        link2 = new IdLink(p, null);
        link2.setWert(2);
        link2.setZielId(data.getCharElement(EigenschaftEnum.ASP.getId()));
        ...
        assertEquals( // Stufe: (10 + 11 + 11) / 2 = 16 + 3 + 2 = 21
                21, prozessor.getHeld().getEigenschaftsWert(EigenschaftEnum.ASP));
        assertEquals( // User Gewählt
        ...
        assertEquals( // Stufe: (10 + 11 + 11) / 2 = 16 + 1 + 2 = 19
                19, prozessor.getHeld().getEigenschaftsWert(EigenschaftEnum.ASP));
        assertEquals( // User Gewählt
                2, getLink(EigenschaftEnum.ASP).getUserLink().getWert());
        assertEquals( // Modis
